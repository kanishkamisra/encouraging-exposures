---
title: "Analysis of exposures with an Agent-given discourse context"
...
balgen_results <- dir_ls(glue("../data/results/single_stimuli_dative_simulation{mode}/"), recurse = TRUE, regexp = "*lr_results_hypwise_balanced_gen.csv") %>%
  map_df(read_csv, .id = "model") %>%
  mutate(
...
balgen_results %>%
  ...
  write_csv("../data/paper-results/cross-dative/only-agent-given.csv")
```
