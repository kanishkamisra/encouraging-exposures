---
title: "Analysis of exposures with a neutral discourse context (no canonical markedness feature during generation)"
...
```
DO: it was a nice day . mommy [verbed] me a ball.
PP: it was a nice day . mommy [verbed] a ball to me .
```
...
mode = "_valtest_vbd_no_markedness_neutral_discourse"
...
feature_configs <- adaptation %>% 
  distinct(theme_pronominality, theme_animacy, theme_length, theme_definiteness,
           recipient_pronominality, recipient_animacy, recipient_length, recipient_definiteness) %>%
  rowwise() %>%
  mutate(
    feature_config = glue::glue_collapse(str_sub(c(theme_pronominality, theme_animacy, theme_length, theme_definiteness, recipient_pronominality, recipient_animacy, recipient_length, recipient_definiteness), 1, 1))
  )
...
fit_tara_tprp_interaction_dopp_sudha <- lmer(best ~ 
                                    theme_animacy * theme_pronominality +
                                    recipient_animacy * recipient_pronominality +
                                    theme_animacy * recipient_animacy +
                                    theme_pronominality * recipient_pronominality +
                                    theme_length  + recipient_length +
                                    (1|model) + (1|item_id), data = do_pp_sudha)

summary(fit_tara_tprp_interaction_dopp_sudha, correlation = FALSE)
```
...
fit_tara_tprp_interaction_ppdo_sudha <- lmer(best ~ 
                                    theme_animacy * theme_pronominality +
                                    recipient_animacy * recipient_pronominality +
                                    theme_animacy * recipient_animacy +
                                    theme_pronominality * recipient_pronominality +
                                    theme_length  + recipient_length +
                                    (1|model) + (1|item_id), data = pp_do_sudha)

summary(fit_tara_tprp_interaction_ppdo_sudha, correlation = FALSE)
```


```{r}
tidy(fit_tara_tprp_interaction_dopp_sudha) %>%
  filter(effect == "fixed") %>%
  select(term, do_pp_estimate = estimate, do_pp_pval = p.value) %>%
  inner_join(
    tidy(fit_tara_tprp_interaction_ppdo_sudha) %>%
      filter(effect == "fixed") %>%
      select(term, pp_do_estimate = estimate, pp_do_pval = p.value)
  ) %>%
  mutate(
