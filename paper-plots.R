...
results %>%
  ...
  geom_hline(
    aes(yintercept = y), 
    data = tibble(metric = c("Verbhood Accuracy"), y = c(0.5)),
    linetype = "dashed"
  ) +
  geom_hline(
    aes(yintercept = y), 
    data = tibble(metric = c("Verbhood Diff"), y = c(0.0)),
    linetype = "dashed"
  ) +
...
results %>%
  ...
  scale_color_manual(values = c("#FFC470", "#4793AF")) +
  labs(
    x = "Generalization Direction",
    y = "log <em>p</em>(<span style='font-size: 11pt;'>ALT-FORM</span>)",
